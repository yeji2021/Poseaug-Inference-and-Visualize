...
def downsample_tensor(X, factor):
    ...
    return np.mean(X[:length].reshape(-1, factor, *X.shape[1:]), axis=1)
                    ...
                    # keypoints : 2d keypoints, poses.value() : 3d predictions
def render_animation(keypoints, keypoints_metadata, poses, skeleton, fps, bitrate, azim, output, viewport,
    ...
    ax_in = fig.add_subplot(1, 1 + len(poses), 1)   # len(poses) : 1 ; Regression
    ...
    for index, (title, data) in enumerate(poses.items()):
        ...
        ax.view_init(elev=15., azim=azim)   # elev : z 높이 , azim = x,y 각도
    ...
    def update_video(i):
        ...
        for n, ax in enumerate(ax_3d):
            ax.set_xlim3d([-radius / 2 + trajectories[n][i, 0], radius / 2 + trajectories[n][i, 0]])    # trajectories : all zeros..
            ax.set_ylim3d([-radius / 2 + trajectories[n][i, 1], radius / 2 + trajectories[n][i, 1]]) '''

        for n, ax in enumerate(ax_3d):
            ax.set_xlim3d([-radius, radius])  # trajectories : all zeros..
            ax.set_ylim3d([-radius / 2 + trajectories[n][i, 1], radius / 2 + trajectories[n][i, 1]])
